...
def maxSatSynthesize(config, sketchFile, reportFile, outerLoop, counter, generator, aux_func, dname, negexSubset,
    ...
    sk.dumpSketchFile(config, skfile, generator, aux_func, dname, domainMap, negexSubset, cexMap["Positive"], 0, 1)
    skfile.close()
    ...
    if res[0].strip() == "SAT":
        ...
        delta, boolFlag = sk.processHoles(config, sketchF, holeFile, 1, reportFile, cexMap, negexSubset, dname, domainPy)
        return delta
    else:
...
def checkPrecision(config, sketchFile, reportFile, outerLoop, counter, domainMap, generator, aux_func, cexMap, domainPy):
    ...
    for dname, culprit in domainMap.items():
        ...
        console.success("reduced counter example found: " + str(curNex))
        ...
        # def dumpSketchFile(config, skfile, generator, aux_func, culpritFun, domainList, curNex, posex, cexGenFlag, maxsatFlag):
        sk.dumpSketchFile(config, skfile, generator, aux_func, dname, domainMap, curNex, cexMap["Positive"], 1, 1)
        skfile.close()
        ...
        if res[0].strip() == "SAT":
            # need to collect the negative example and add it to the cexMap
            holeFile = config["basepath"] + "/temp/hole.xml"
            newCex, exampleRes = sk.processHoles(config, sketchF, holeFile, 0, reportFile, cexMap, curNex, dname, domainPy)
            # delta = sk.processHoles(config, sketchF, holeFile, 1, reportFile, cexMap, curNex, dname, domainPy)
            ...
            # return "SAT", dname, curNex, newCex
...
def checkSoundness(config, domainMap, logFile, cexMap):
    ...
    return res
    # if res == "UNSAT":
    #     console.success("UNSAT, so closing the shop!")
    #     with open(logFile) as f:
    #         f.write("UNSAT in soundness check!\n")
    # else:
    #     posEx = soundnessCheck.processPosEx(config)
    #     print(posEx)
    #     labelName = "posex" + str(len(cexMap["Positive"]))
    #     cexMap["Positive"][labelName] = posEx
    #     with open(logFile) as f:
    #         f.write("Generated positive example: " + str(posEx).replace("[", "").replace("]", ""))
